...
main(int argc, char **argv) {
    opts o;
    int c;
    while ((c = getopt(argc, argv, "p:c:o:l:s:i:r:f:")) != -1) {
        switch (c) {
    ...
    /* One DRAM buffer per in-flight slot, reused across requests: a real
     * client holds its staging buffers, and reallocating per request churns
     * addresses in a way the fabric's MR cache does not like. */
    const size_t slotBytes = (size_t)o.layers * o.lsize;
    std::vector<std::vector<uint8_t>> bufs(o.inflight);
    for (auto &b : bufs) b.assign(slotBytes, 0x5a);
    ...
    struct unitDesc {
        nixl_meta_dlist_t loc{DRAM_SEG};
    ...
    for (int i = 0; i < o.objs; i++) {
        if (o.fold) {
            ...
            for (int l = 0; l < o.layers; l++) {
                u.loc.addDesc(nixlMetaDesc(0, o.lsize, 0, nullptr)); /* addr set later */
                u.rem.addDesc(
            ...
            for (int l = 0; l < o.layers; l++) {
                unitDesc u;
                u.loc.addDesc(nixlMetaDesc(0, o.lsize, 0, nullptr));
                u.rem.addDesc(
                    nixlMetaDesc((uintptr_t)l * o.lsize, o.lsize, 9000 + i, mds[i]));
                units.push_back(std::move(u));
            }
        ...
        auto bindBufs = [&](unitDesc &u, int slot) {
            ...
            for (auto it = u.loc.begin(); it != u.loc.end(); ++it, ++k)
                const_cast<nixlMetaDesc &>(*it).addr =
                    (uintptr_t)(bufs[slot].data() + (size_t)k * o.lsize);
        };
