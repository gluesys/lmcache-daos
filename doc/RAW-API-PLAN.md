...
**이 설명은 표의 모양과 DAOS 배치 규칙에서 나온 것이지 측정한 것이 아니다.**
확정하려면 타깃별 I/O 분포를 보아야 한다.
...
버릴 것은 아니다. 고쳐야 할 것이 분명해졌다 — **페이로드를 여러 dkey 로 쪼개
스트라이핑을 되찾거나, DFS 가 쓰는 DAOS array API 를 저수준에서 그대로 쓰는
것**이다. 그때 다시 이 표를 뜨면 된다. 지금 상태로는 큰 객체에서 명백한 회귀다.
