...
buffer mix-up is reported as such instead of looking like random corruption.
    ...
    DAOS_TEST_POOL=gdspool DAOS_TEST_CONT=kvlmc \\
        python3 tests/test_rawio_integrity.py [chunk_MiB] [threads] [rounds]
"""
    ...
    def worker(tid: int) -> None:
        ...
        for r in range(rounds):
            ...
            try:
                ...
                dfs.write_obj_from(obj, len(header), chunk, src)
            finally:
                dfs.close_obj(obj)
            ...
            try:
                got = dfs.read_obj_into(obj, len(header), chunk, dst)
            finally:
                dfs.close_obj(obj)
