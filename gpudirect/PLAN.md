# DaosGdsBackend 구현 계획 — 착수 판정: 부정 → **번복, 구현됨 (실험적)**

> **2026-09-06 현재 상태.** 아래 "착수하지 않는다" 판정은 `ucx+rc_v` 전송에서만 성립하는
> 결론이었다. `ofi+verbs` 로 바꾸면 GDS 가 스테이징의 0.86~1.1배로 회복돼 재개 조건이
> 충족됐고, Phase 1~3 을 구현·검증했다(`lmcache_daos/gds_backend.py`). 다만 결과물은
> **실험적**이다 — 상류 미병합 의존, 복제 컨테이너 쓰기 결함, 단일 GPU 검증. 저장소
> README 의 "모드별 성숙도" 를 볼 것. 아래 원문은 판정이 어떻게 뒤집혔는지 남기기 위해
> 그대로 둔다.
