...
>
> **그리고 더 중요한 것이 나왔다.** 운영 경로의 LMCache 계측에서 store 의 DAOS 쓰기가
> `put_time` 0.13 ms 인데 `offload_time` 이 93.28 ms 다(약 700배). 리트리브도 3.46 GB/s 로
> raw DFS 스테이징(35 GB/s)의 1/10 이다. **병목은 스토리지·전송이 아니라 LMCache offload
> 파이프라인이다.** 전송 계층을 바꿔서 얻을 수 있는 몫이 전체의 몇 %도 되지 않는다.
> 다음 작업은 그쪽이어야 한다.
