...
### 최종 수치 (chunk 4 MiB + `RP_2G4`)

| 워커 | arm | GB/s | 청크 ms | cyc/byte | DRAM 배수 |
|---|---|---|---|---|---|
| 1 | `gpu` | 7.49 | 4.48 | 0.672 | **0.10** |
| 1 | `pinnedcopy` | **10.46** | 3.21 | 0.677 | 1.95 |
| 1 | `pinned` | 13.36 | 2.51 | 0.473 | 1.11 |
| 16 | `gpu` | 15.00 | 35.80 | 4.960 | **0.30** |
| 16 | `pinnedcopy` | **25.64** | 20.94 | 2.854 | 2.02 |
| 16 | `pinned` | 25.89 | 20.74 | 4.423 | 1.14 |
| 16 | `hostcopy` | 5.80 | 92.56 | 1.063 | 4.73 |

**GPU-direct 는 이 환경에서 대역폭과 지연 모두 진다** — 16 워커에서 스테이징의 0.59배,
단일 워커에서 0.72배. `cyc/byte` 도 16 워커에서는 오히려 불리한데, 느린 만큼 폴링 스레드가
바이트당 더 오래 도는 탓이라 이 지표는 처리량과 얽혀 있어 단독으로 읽으면 안 된다.

**유일하게 남는 이득은 호스트 DRAM 트래픽이다.** 전달 바이트당 0.10 vs 1.95 (1워커,
**20×**), 0.30 vs 2.02 (16워커, **6.7×**). 청크 크기·oclass·워커 수를 바꿔도 방향이
유지되는 지표는 이것뿐이다. 분해도 맞물린다 — `pinned`(H2D 없음)의 DRAM write 는 NIC 이
호스트로 DMA 한 8 GiB 이고, `pinnedcopy` 는 복사 엔진이 그것을 되읽는 몫을 더한다.
`gpu` 는 둘 다 없다.
...
- **cell1 의 `0000:06:00.0` 은 bdev_list 에서 제외되어 있다.** SMART 는 정상(critical_warning
  0, media_errors 0)인데 DAOS 가 `load blobstore failed -1025` 로 그 타깃의 blobstore 를
  올리지 못한다. 반복 재구성 중 발생했고 저수준 wipe 가 필요할 것으로 보인다. 그래서 현재
  cell1 은 NVMe 7개, cell2 는 8개로 비대칭이다.
