...
1. **목적지 버퍼 가설을 가른다** — 재현기에서 `_get_sync` 의 MemoryObj alias 대신 중간
   `bytearray` 로 읽고 복사하도록 바꿔 동시 조건(키 16개)으로 같은 규모를 돌린다.
   사라지면 원인 확정(수정은 read 경로 복사 1회 추가 또는 버퍼 등록), 남으면 DFS 바인딩
   또는 DAOS array 계층으로 내려간다.
