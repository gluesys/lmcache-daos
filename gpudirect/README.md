...
##### raw 테스트가 80/80 통과한 이유

`../tests/test_rawio_integrity.py` 는 16 스레드가 **각자 write→read 를 반복**한다. 재현기는
`batched_get` 으로 **16개 읽기를 한꺼번에 발사**한다. 후자가 훨씬 조밀한 동시성이고, 그래서
raw 테스트가 통과한 것으로 보인다 — 결함의 부재가 아니라 접근 패턴 차이다.

##### 다음 단계

0. ~~같은 호스트에서 main vs 브랜치 비교~~ → 완료. **코드는 원인이 아니다.**
1. ~~목적지 버퍼 가설~~ → 완료. **기각.** `bytearray` 가 더 나쁘다.
2. **raw 테스트를 버스트형으로 바꾼다** — 쓰기 완료 후 배리어, 그 다음 모든 읽기를 동시
   발사. 재현되면 **LMCache·Python 객체가 전혀 없는 순수 DAOS 재현기**가 되고, 상류가
   LMCache 가 아니라 DAOS 가 된다. 지금까지의 증거는 모두 그 방향을 가리킨다.
