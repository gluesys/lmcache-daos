...
### Concurrency (8 GiB / 32 MiB 청크, 워커 수 sweep)
...
포화점에서 격차가 오히려 벌어진다: **2.44× (12.16 vs 4.94 GB/s)**, 워커당 지연은
**2.46× 낮다**(88 vs 218 ms). 두 arm 이 같은 서버를 때리는데 `pinnedcopy` 는 약 5 GB/s
에서, `gpu` 는 약 12 GB/s 에서 멈춘다 — 즉 스테이징 경로의 상한은 **클라이언트 쪽**
(호스트 DRAM + 복사 엔진 + 소켓 횡단)이고, 그것을 없애면 천장이 2.4배 올라간다.

`gpu` 의 12.1 GB/s 평탄부가 무엇의 한계인지는 아직 모른다. 400G NIC 의 이론치보다 훨씬
낮으므로 서버 측(엔진 xstream CPU 포화가 관측된 바 있다) 또는 `SYS` 배치가 후보다.
그리고 `pinnedcopy` 는 워커 내부에서 read 와 H2D 를 직렬로 수행한다 — 32 워커에서는
워커 간 겹침이 생기므로 측정된 평탄부는 스테이징 경로의 합리적 추정이지만, 이중 버퍼로
설계한 전용 스테이저라면 이보다 나을 수 있다.
...
- **측정 범위가 read 경로에 한정된다.** 대역폭·지연·cycles/byte·DRAM 트래픽·concurrency
  ...
  고정하면 얼마나 달라지는지도 모른다. `gpu` 의 12.1 GB/s 평탄부가 서버 한계인지
  토폴로지 한계인지도 미규명이다.
