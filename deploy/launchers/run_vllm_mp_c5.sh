...
MP_EXTRA=${MP_EXTRA:-}   # extra lmcache mp server flags, e.g. "--l2-prefetch-max-in-flight 16"
...
python3 -m lmcache_daos.mp.server --host localhost --port $MP_PORT --chunk-size 256 \
  ...
  --l2-adapter '{"type":"daos","pool":"$POOL","container":"$CONT","root":"$ROOT","workers":$WORKERS,"status_interval_s":$STATUS_S,"load_schedule":"${SCHED:-fifo}"}' \
  --disable-observability ${MP_EXTRA:-} > >(sed -u 's/^/[mp-server] /') 2>&1 &
SRV=\$!
...
podman run -d --name vllm-daos --net host --security-opt label=disable --device nvidia.com/gpu=all --ipc host \
  ...
  -e VLLM_USE_FLASHINFER_SAMPLER=0 -e D_LOG_MASK=${D_LOG_MASK:-WARN} -e D_LOG_FILE=${D_LOG_FILE:-/tmp/daos_client.log} -e DAOS_MP_GC=${DAOS_MP_GC:-} \
