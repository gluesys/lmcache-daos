...
mercury/UCX 송신 경로에 머묾, 서버 무죄, 쓰기+15 s 에 풀림; §7.7b). `ofi+tcp` 로 바꾸면 0/8 로 사라져 **UCX 한정**(§7.7c). `ofi+verbs;ofi_rxm` 은 스톨 0/6 에 대역폭 동일(35~38 GB/s), 정합성 통과 → 클러스터를 verbs 로 전환(§7.7d). L1 < working set 의 p95 꼬리는 대역폭 포화 큐잉이며
